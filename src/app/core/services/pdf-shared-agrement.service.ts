  ...
  async fillPdf(payload: any): Promise<string> {
    try {
      ...
      doc.setData({
        ...
        'No. of Shares -> 1,000': payload.shareholders[0].shareDetailsNoOfShares.toString(),
        ...
        'Director 1 English name': `${payload.directors[0].name} ${payload.directors[0].surname}`,
        'Director 2 English name':payload.directors?.[1]?`${payload.directors[1].name || ''}${payload.directors[1].surname || ''}`: '',
      ...
      this.generatedFilename = `${payload.companyInfo[0].business_name}_ShareCertificate.docx`;
